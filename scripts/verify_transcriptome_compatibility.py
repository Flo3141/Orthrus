...
OUTPUT_FILE = Path("/beegfs/prj/RNA_NLP/FlorianMasterThesis/code/data/transcriptome_compatibility_report.txt")
...
report_lines = [
    "=" * 65,
    "           TRANSCRIPTOME COMPATIBILITY CHECK RESULTS         ",
    "=" * 65,
    f"Total transcripts:                   {total}",
    "-" * 65,
    "1. GTF Comparison (Ensembl 108 vs. hIPSC_CM sequences):",
    f"  Found in GTF:                      {found_in_gtf} ({found_in_gtf/total*100:.2f}%)",
    f"  Exact length match:                {exact_length_match} ({exact_length_match/found_in_gtf*100:.2f}% of found)",
]

if length_diffs:
    arr = np.array(length_diffs)
    report_lines.extend([
        "  Length difference (seq_len - gtf_len):",
        f"    Mean: {np.mean(arr):.2f} bp | Median: {np.median(arr):.1f} bp",
        f"    |Diff| == 0 bp: {np.sum(arr == 0) / len(arr) * 100:.2f}%",
    ])

report_lines.extend([
    "-" * 65,
    "2. TargetScan 8.0 3' UTR Comparison:",
    f"  Transcripts with TargetScan data:  {ts_tx_with_predictions} ({ts_tx_with_predictions/total*100:.2f}%)",
])

if tx_without_cds > 0:
    report_lines.append(f"  Of which without annotated CDS:    {tx_without_cds}")

report_lines.append(f"  Total miRNA binding sites:         {total_sites}")

if total_sites > 0:
    fit_pct = fitting_sites / total_sites * 100
    ov_pct = overflowing_sites / total_sites * 100
    report_lines.extend([
        f"  Fit completely in 3' UTR:          {fitting_sites} ({fit_pct:.2f}%)",
        f"  Overflow beyond 3' UTR:            {overflowing_sites} ({ov_pct:.2f}%)",
        f"  Transcripts with >= 1 overflow:    {transcripts_with_overflow} ({transcripts_with_overflow/ts_tx_with_predictions*100:.2f}%)",
    ])

    if overflow_distances:
        ov_arr = np.array(overflow_distances)
        report_lines.extend([
            "  Overflow distance for overflows:",
            f"    Mean: {np.mean(ov_arr):.1f} bp | Median: {np.median(ov_arr):.1f} bp | Max: {np.max(ov_arr)} bp",
        ])
